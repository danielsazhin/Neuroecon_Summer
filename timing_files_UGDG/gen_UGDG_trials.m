...
% Make UGDG timing files. 
% Minimizing ITI and ISI since this is not a scanner task. 
...
one_second_trials_ISI = 36;
%twopointfive_second_trials_ISI = 10;
%four_second_trials_ISI = 4;

% If you want to use the ITI and ISI code above, you need to uncomment the
% respective code in the ITI and ISI sections.
...
num_partners = trials/4;

% 1 will define faculty camp 2: student camp 3: faculty noncamp 4: student
% noncampt
a(1:num_partners, 1) = 1;
b(1:num_partners, 1) = 2;
c(1:num_partners, 1) = 3;
d(1:num_partners, 1) = 4;
...
ITI_list = repmat(1,1,one_second_trials);
...
ISI_list = repmat(1,1,one_second_trials_ISI);
...
A = combnk([0.05 .10 .25 .50], 2); % 10 possible combinations we will use for proposers
...
% 
% A = combnk(0.06:0.13:0.6,2); % 10 possible combinations we will use for proposers
% B = fliplr(A); % To counterbalance.
% Recipient_Options = [A;B;A]; % Now the options are counterbalanced
% 
...
for aa = 1:runs
    ...
    for jj=1:subjects
        ...
        Proposer_Ind = find(participant(:,2) < 5);
...
%         end
        ...
        % given participant.
        participant = [proposer]; % Concatenated
        % participant = [recipient]; % Concatenated
