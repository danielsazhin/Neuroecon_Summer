...
#instructions
instruct_screen = visual.TextStim(win, text="Welcome to Let's Make a Deal!\n\nIn this task you will be making decisions involving sending money to another person.\n\nPress the 1 key with your right pointer finger to continue", pos = (0,1), wrapWidth=20, height = 1.2)
instruct_screen2 = visual.TextStim(win, text = 'At the beginning of each round, you will be given between $16-32.\n\nYou will decide how much to share with your partner.', pos = (0,1), wrapWidth=20, height = 1.2)
#instruct_screen3 = visual.TextStim(win, text = 'Sometimes you will receive offers from another person, which you can either accept or reject.\n\nIf you reject the offer, both you and your partner will earn $0.',pos = (0,1), wrapWidth=20, height = 1.2)
instruct_screen4 = visual.TextStim(win, text = 'On all rounds, if you would like to select the option on the left side of the screen, press the 1 key (pointer finger).\n\nIf you would like to select the option on the right side of the screen, press the 2 key (middle finger).\n\n"More" and "Less" offers will switch sides between rounds.',pos = (0,1), wrapWidth=20, height = 1.2)
instruct_screen5 = visual.TextStim(win, text = "Your partner will be a faculty member or a student, from within the summer camp or from a University of Pennsylvania person.", pos = (0,5), wrapWidth=20, height = 1.2)
instruct_screen6 = visual.TextStim(win, text = "\nOne decision will be randomly selected at the end of the study and the interaction will occur in real life.\n\n You will be paid based on its result.",pos = (0,1),wrapWidth=20,height=1.2)
instruct_screen7 = visual.TextStim(win, text = 'Do you have any questions?',pos = (0,1),wrapWidth=20,height=1.2)
...
trial_data_1 = [r for r in csv.DictReader(open('timing_files_UGDG/Subject_' + subj_id + '_run_1.csv','rU'))]
trial_data_2  = [r for r in csv.DictReader(open('timing_files_UGDG/Subject_' + subj_id + '_run_2.csv','rU'))]
...
faces_faculty_ingroup = [img for img in os.listdir('Images/experiment/faces_faculty_ingroup') if 'h' in img]
faces_student_ingroup = [img for img in os.listdir('Images/experiment/faces_student_ingroup') if 'h' in img]

random.shuffle(faces_faculty_ingroup)
random.shuffle(faces_student_ingroup)

run1_faces=faces_faculty_ingroup[0:7]+faces_student_ingroup[0:7]
run2_faces=faces_faculty_ingroup[8:15]+faces_student_ingroup[8:15]
...
def do_run(run, trials):
    ...
    for trial in trials:
        ...
        if trial['Block'] == '4':
        ...
        elif trial['Block'] == '2': #Outgroup Student
            while timer.getTime() < 1:
                cue = os.path.join(imagepath,"outgroup_student.png")
                cueStim.setImage(cue)
                cue_Onset = globalClock.getTime()
                trials.addData('cue_Onset',cue_Onset)
                cueStim.draw()
            ...
            trial['Block'] == '1' # faculty
            while timer.getTime() < 1:
                cue = os.path.join(imagepath,"outgroup_faculty.png")
                cueStim.setImage(cue)
                cue_Onset = globalClock.getTime()
                trials.addData('cue_Onset',cue_Onset)
                cueStim.draw()
